...
//         //clang-format off
//         nRet = MV_CC_EnumDevices(MV_GIGE_DEVICE, &stDeviceList);  if (nRet!=MV_OK) { printf("Find No Devices!\n"); break;}
...
//         if (deviceInfo->nDevTypeInfo != MV_GIGE_DEVICE) { printf("Find No MV_GIGE_DEVICE Devices!\n"); break;}
...
//         else printf("Warning: Get Packet Size fail nRet [0x%x]!", nPacketSize);

//         nRet = MV_CC_SetEnumValue(mvHandle, "TriggerMode", 1);  if (nRet!=MV_OK){ printf("Set Trigger Mode :! nRet [0x%x]\n", nRet); break;}
//         nRet = MV_CC_SetIntValueEx(mvHandle, "GevSCPSPacketSize", nPacketSize); if (nRet!=MV_OK)  {qDebug()<<mvHandle <<"GevSCPSPacketSize:<<mvHandle"<< nPacketSize;};
